...
// inner width, length & heigth
wi=60;	
li=73;
h=20;
// wall thickness
th=2;
// Pot Adjustment Hole Radius
potr=2.7;
...
module led_holes(count=4) {
    ...
    for (i = [0 : count-1]) {
        ...
        translate([-int_w/2-th+letterDepth, int_l/2 - led_offset + i*led_spacing + led_diameter/2, pcb_foot_h+pcb_thickness+.6])
...
module audio_jacks()
{
        ...
        translate([-int_w/2-th+letterDepth, int_l/2 - audiojack1_offset + 3.5, pcb_foot_h+pcb_thickness+th+6.5])
        ...
        translate([-int_w/2-th+letterDepth, int_l/2 - audiojack2_offset + 3.5, pcb_foot_h+pcb_thickness+th+6.5])
        rotate([90, 0, 270])  // orient text to face front
            linear_extrude(height=2.0)
